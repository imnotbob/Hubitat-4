...
preferences{
	page ((sNM): "mainPage", title: sBLANK, install: true, uninstall: false)
	page ((sNM): "NotificationPage", title: sBLANK, install: false, uninstall: false, nextPage: "mainPage")
	page ((sNM): "ConfigPage", title: sBLANK, install: false, uninstall: false, nextPage: "mainPage")
	page ((sNM): "AdvConfigPage", title: sBLANK, install: false, uninstall: false, nextPage: "mainPage")
	page ((sNM): "RestrictionsPage", title: sBLANK, install: false, uninstall: false, nextPage: "mainPage")
	page ((sNM): "SettingsPage", title: sBLANK, install: false, uninstall: true, nextPage: "mainPage")
}

def mainPage(){
	if(settings.logInfo==null) app.updateSetting("logInfo", [(sVL):sTRUE, (sTY):sBOOL])
	if(settings.logWarn==null) app.updateSetting("logWarn", [(sVL):sTRUE, (sTY):sBOOL])
	if(settings.logError==null) app.updateSetting("logError", [(sVL):sTRUE, (sTY):sBOOL])

	dynamicPage((sNM): "mainPage"){
		installCheck()
		if((String)state.appInstalled=='COMPLETE'){
			section(UIsupport("logo")){
				if((Boolean)settings.pushovertts || (Boolean)settings.musicmode || (Boolean)settings.speechmode || (Boolean)settings.echoSpeaks2) href((sNM): "NotificationPage", title: "${UIsupport("configured")} Setup Notification Device(s)", required: false, page: "NotificationPage", description: "Select TTS and PushOver Devices")
				else href((sNM): "NotificationPage", title: "${UIsupport("attention")} Setup Notification Device(s)", required: false, page: "NotificationPage", description: "Select TTS and PushOver Devices")

				if(settings.whatAlertSeverity || settings.whatPoll || settings.alertCustomMsg) href((sNM): "ConfigPage", title: "${UIsupport("configured")} Weather Alert Settings", required: false, page: "ConfigPage", description: "Change default settings for weather alerts to monitor")
				else  href((sNM): "ConfigPage", title: "${UIsupport("attention")} Weather Alert Settings", required: false, page: "ConfigPage", description: "Change default settings for weather alerts to monitor")

				if(settings.myWeatherAlert || settings.whatAlertUrgency || settings.whatAlertCertainty) href((sNM): "AdvConfigPage", title: "${UIsupport("configured")} Advanced Alert Settings", required: false, page: "AdvConfigPage", description: "Add additional detailed weather settings to monitor")
				else href((sNM): "AdvConfigPage", title: "Advanced Alert Settings", required: false, page: "AdvConfigPage", description: "Add additional detailed weather settings to monitor")

				if((Boolean)settings.modesYes || (Boolean)settings.switchYes || (Boolean)settings.modeSeverityYes || (Boolean)settings.pushoverttsalways) href((sNM): "RestrictionsPage", title: "${UIsupport("configured")} Restrictions", required: false, page: "RestrictionsPage", description: "Setup restriction options")
				else href((sNM): "RestrictionsPage", title: "Restrictions", required: false, page: "RestrictionsPage", description: "Setup restriction options")
				href((sNM): "SettingsPage", title: "Settings", required: false, page: "SettingsPage", description: "Modify NOAA Weather Alerts Application Settings, Logging, Test")
...
def NotificationPage(){
	...
	dynamicPage((sNM): "NotificationPage"){
		section(UIsupport("logo")){
			...
			input((sNM): "musicmode", (sTY): sBOOL, defaultValue: false, title: "Use audioNotification Speaker(s) for TTS?", description: "audioNotification Speaker(s)?", submitOnChange: true)
			...
			// Speech Speakers
			input((sNM): "speechmode", (sTY): sBOOL, defaultValue: false, title: "Use speechSynthesis Speaker(s) for TTS? (Google, Alexa TTS, etc)", description: "Speech Speaker(s)?", submitOnChange: true)
			if((Boolean)settings.speechmode){
				...
				input((sNM): "speechdelay", (sTY): sBOOL, defaultValue: false, title: "Place delays between commands to speechSynthesis Speaker(s)", description: "Speech Speaker(s)?", submitOnChange: true)
			} else{
			...
			// Echo Speaks devices
			input ((sNM): "echoSpeaks2", (sTY): sBOOL, defaultValue: false, title: "Use Echo Speaks device(s) for Announcement TTS?", description: "Echo Speaks device?", submitOnChange: true)
			...
			input ((sNM): "UsealertSwitch", (sTY): sBOOL, title: "Use a switch to turn ON with Alert?", required: false, defaultValue: false, submitOnChange: true)
			if((Boolean)settings.UsealertSwitch){
				input ((sNM): "alertSwitch", (sTY): "capability.switch", title: "Select a switch to turn ON with Alert?", multiple: false, required: false, defaultValue: false, submitOnChange: true)
				input ((sNM):"alertSwitchOff", (sTY): sBOOL, title: "Turn off switch when all Alerts expire?", required: false, defaultValue: false, submitOnChange: true)
				input ((sNM):"alertSwitchWeatherType", (sTY): sBOOL, title: "Turn off switch if certain weather alert types expire?", required: false, defaultValue: false, submitOnChange: true)
			...
			input ((sNM): "disableTile", (sTY): sBOOL, defaultValue: false, title: "Disable updates of Tile Device to display alerts?", description: "Disable tile device?", submitOnChange: true)
		}
	}
}
	...
	dynamicPage((sNM): "ConfigPage"){
		section(UIsupport("logo")){
			...
			if((Boolean)settings.repeatYes){
				input ((sNM):"repeatTimes", (sTY): sNUMBER, title: "Number of times to repeat the alert?", require: false, defaultValue: 1, submitOnChange:true)
				input ((sNM):"repeatMinutes", (sTY): sNUMBER, title: "Number of minutes between each repeating alert?", require: false, defaultValue: 15, submitOnChange:true)
			}
			input ((sNM): "useCustomCords", (sTY): sBOOL, title: "Use Custom Coordinates?", require: false, defaultValue: false, submitOnChange: true)
			if((Boolean)settings.useCustomCords){
				...
				input ((sNM):"customlatitude", (sTY):"text", title: "Latitude coordinate:", require: false, defaultValue: "${location.latitude}", submitOnChange: true)
				input ((sNM):"customlongitude", (sTY):"text", title: "Longitude coordinate:", require: false, defaultValue: "${location.longitude}", submitOnChange: true)
			}
			input ((sNM):"useAlertIntro", (sTY): sBOOL, title: "Use a pre-notification message for TTS device(s)?", require: false, defaultValue: false, submitOnChange: true)
			if((Boolean)settings.useAlertIntro) input ((sNM):"AlertIntro", (sTY): "text", title: "Alert pre-notification message:", require: false, defaultValue:"Attention, Attention")
			input ((sNM): "alertCustomMsg", (sTY): "text", title: "Custom Alert Message (use customization instructions):", require: false, defaultValue: "{alertseverity} Weather Alert for the following counties:{alertarea}{alertdescription} This is the end of this Weather Announcement.", submitOnChange: true)
		}
...
def AdvConfigPage(){
	...
	dynamicPage((sNM): "AdvConfigPage"){
		section(UIsupport("logo")){
			...
			input ((sNM):"myWeatherAlert", (sTY):sENUM, title: "Filter results for specific Weather event(s)?", required: false, multiple: true, submitOnChange: true, options: (List)state.eventTypes)
			input ((sNM): "whatAlertUrgency", (sTY): sENUM, title: "Poll only for a specific Alert Urgency: ", multiple: true, submitOnChange: true,
				options: [
					"unknown": "Unknown",
					//"past": "Past",
					"immediate": "Immediate",
			...
			input ((sNM): "whatAlertCertainty", (sTY): sENUM, title: "Poll only for specific Alert Certainty: ", required: false, multiple: true, submitOnChange: true,
				options: [
					"unknown": "Unknown",
					//"unlikely": "Unlikely",
					"possible": "Possible",
...
def RestrictionsPage(){
	...
	dynamicPage((sNM): "RestrictionsPage"){
		section(UIsupport("logo")){
			...
			input ((sNM):"modesYes", (sTY):sBOOL, title: "Enable restriction of notifications by current mode(s)?", required: true, defaultValue: false, submitOnChange: true)
			if((Boolean)settings.modesYes) input((sNM):"modes", (sTY): "mode", title: "Restrict notifications when current mode is:", multiple: true, required: false, submitOnChange: true)
			input ((sNM):"switchYes", (sTY):sBOOL, title: "Restrict notifications using a switch?", required: true, defaultValue: false, submitOnChange: true)
			if((Boolean)settings.switchYes) input ((sNM):"restrictbySwitch", (sTY):"capability.switch", title: "Use a switch to restrict notfications?", required: false, multiple: false, defaultValue: null, submitOnChange: true)
			paragraph "<br>"
			...
			input ((sNM):"modeSeverityYes", (sTY):sBOOL, title: "Ignore restrictions for certain severity types?", required: false, defaultValue: false, submitOnChange: true)
			if((Boolean)settings.modeSeverityYes) input ((sNM): "modeSeverity", (sTY): sENUM, title: "Severity option(s) that will ignore restrictions: ",
				options: [
					"Minor": "Minor",
					"Moderate": "Moderate",
					"Severe": "Severe",
			...
			input ((sNM):"modeWeatherType", (sTY):sBOOL, title: "Ignore restrictions for certain weather types?", required: false, defaultValue: false, submitOnChange: true)

			if((Boolean)settings.modeWeatherType) input ((sNM): "WeatherType", (sTY): sENUM, title: "Select weather type to ignore restrictions: ", required: true, multiple:true, submitOnChange: true, options: (List)state.eventTypes)
			paragraph "<hr>"
			if((Boolean)settings.pushovertts) input ((sNM):"pushoverttsalways", (sTY):sBOOL, title: "Enable Pushover notifications even when restricted?", required: false, defaultValue: false, submitOnChange: true)
...
//			if((Boolean)settings.logEnable) input "logMinutes", sNUMBER, title: "Log for the following number of minutes (0=logs always on):", range: "0..300", required: false, defaultValue:15, submitOnChange: true
			...
			if((Boolean)settings.runTest){
				app.updateSetting("runTest",[(sVL):sFALSE,(sTY):sBOOL])
			...
			if((Boolean)settings.init){
				app.updateSetting("init",[(sVL):sFALSE,(sTY):sBOOL])
			...
			if((Boolean)settings.getAPI){
				getAlertMsgSync()
				app.updateSetting("getAPI",[(sVL):sFALSE,(sTY):sBOOL])
				app.removeSetting("getAPI")
				app.updateSetting("debug",[(sVL):sFALSE,(sTY):sBOOL])
		...
		section(){
			...
			label ((sNM): sNM, title: "Name", state: "complete", defaultValue: app.name, required: false)
		}
	}
}
...
//			alertmsg=[]
			...
			if(msgMap!=null){
				Boolean useexpired= (Boolean)settings.debug
...
void checkState(){
	atomicState.testmsg=false
	if(settings.whatPoll==null) app.updateSetting("whatPoll",[(sVL):"5",(sTY):sENUM])
	if(settings.logInfo==null) app.updateSetting("logInfo", [(sVL):sTRUE, (sTY):sBOOL])
	if(settings.logWarn==null) app.updateSetting("logWarn", [(sVL):sTRUE, (sTY):sBOOL])
	if(settings.logError==null) app.updateSetting("logError", [(sVL):sTRUE, (sTY):sBOOL])
	if(settings.logDebug==null) app.updateSetting("logDebug", [(sVL):sFALSE, (sTY):sBOOL])
...
//	if(settings.logEnable==null) app.updateSetting("logEnable",[(sVL):sFALSE,(sTY):sBOOL])
	...
	if(settings.repeatTimes){
		...
		app.updateSetting("repeatTimes",[(sVL):1,(sTY):sNUMBER])
		t0=1
	}
	state.rptNum=t0

	if(settings.repeatMinutes==null || settings.repeatMinutes < 0){
		app.updateSetting("repeatMinutes",[(sVL):15,(sTY):sNUMBER])
	}
...
//	app.updateSetting("logEnable",[(sVL):sFALSE,(sTY):sBOOL])
	...
	app.updateSetting("logDebug", [(sVL):sFALSE, (sTY):sBOOL])
	app.updateSetting("logTrace", [(sVL):sFALSE, (sTY):sBOOL])
	app.updateSetting("logInfo", [(sVL):sFALSE, (sTY):sBOOL])
}
